...
def clamp_and_normalize(img, minmax):
    """
    Clamp and normalize image values to be within [min_val, max_val], then normalize to [0, 1].
    """
    img = torch.clamp(img, minmax[0]/255, minmax[1]/255)
    img = (img - minmax[0]/255) / (minmax[1]/255 - minmax[0]/255)
    return img
...
def get_transform(opt, params=None, grayscale=False, method=Image.BICUBIC, convert=True):
    ...
    if convert:
        ...
        # Add clamp and normalize transform # it should be positioned bofre ToTensor (it conver 0-255 to 0-1)
        if opt.A_minmax != None:
            transform_list.append(transforms.Lambda(lambda img: clamp_and_normalize(img, opt.A_minmax)))
        if grayscale:
