...
class MaskDINODecoder(nn.Module):
    ...
    def __init__(
            ...
            *,
            ...
            pretrained_weight: float = 0.5,
            pretrained_mode: str = 'concat',
    ):
        """
        ...
        if self.mask_classification:
            if self.pretrained_mode == 'concat':
                hidden_dim_addition = hidden_dim
            else:
                hidden_dim_addition = 0
            if self.semantic_ce_loss:
                self.class_embed = nn.Linear(hidden_dim+hidden_dim_addition, num_classes+1)
            else:
                self.class_embed = nn.Linear(hidden_dim+hidden_dim_addition, num_classes)
        ...
        if self.pretrained_mode != None:
            if self.pretrained_model != None:
                self.decoder_pretrained = TransformerDecoder(decoder_layer, self.num_layers, decoder_norm,
                                                return_intermediate=return_intermediate_dec,
                                                d_model=hidden_dim, query_dim=query_dim,
                                                num_feature_levels=self.num_feature_levels,
                                                dec_layer_share=dec_layer_share,
                                                )
                self.decoder_pretrained.bbox_embed = self.bbox_embed
                pretrained_weights = torch.load(self.pretrained_model)
                decoder_weights = {k.replace('sem_seg_head.predictor.decoder.', ''): v for k, v in pretrained_weights['model'].items() 
                                if k.startswith('sem_seg_head.predictor.decoder.')}
                self.decoder_pretrained.load_state_dict(decoder_weights)
            else:
                raise ValueError("pretrained_model이 None입니다. pretrained_mode를 사용하려면 pretrained_model 경로를 지정해야 합니다.")
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        ret["pretrained_model"] = cfg.MODEL.MaskDINO.PRETRAINED_MODEL
        ret["pretrained_weight"] = cfg.MODEL.MaskDINO.PRETRAINED_WEIGHT
        ret["pretrained_mode"] = cfg.MODEL.MaskDINO.PRETRAINED_MODE
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        if self.two_stage:
            ...
            output_memory = self.enc_output_norm(self.enc_output(output_memory))    #[bs, 21824, 256] -> [bs, 21824, 256]
            if self.pretrained_mode == 'concat':
                enc_outputs_class_unselected = self.class_embed(torch.cat([output_memory, output_memory], dim=-1))
            else:
            ...
            if self.pretrained_mode == 'concat':
                outputs_class, outputs_mask = self.forward_prediction_heads_concat(tgt_undetach.transpose(0, 1), tgt_undetach.transpose(0, 1), mask_features)
            else:
        ...
        if self.initial_pred:
            if self.pretrained_mode == 'concat':
                outputs_class, outputs_mask = self.forward_prediction_heads_concat(tgt.transpose(0, 1), tgt.transpose(0, 1), mask_features, self.training)
            else:
        ...
        if self.pretrained_mode != None:
            ...
            hs_pretrained, references_pretrained = self.decoder_pretrained(
                tgt=tgt.transpose(0, 1),
                memory=src_flatten.transpose(0, 1),
                memory_key_padding_mask=mask_flatten,
                pos=None,
            ...
            for i, (output, output_pretrained) in enumerate(zip(hs, hs_pretrained)):
                if self.pretrained_mode == 'concat':
                    outputs_class, outputs_mask = self.forward_prediction_heads_concat(output.transpose(0, 1), output_pretrained.transpose(0, 1), mask_features, self.training or (i == len(hs)-1))
                elif self.pretrained_mode == 'weight':
                    outputs_class, outputs_mask = self.forward_prediction_heads_weight(output.transpose(0, 1), output_pretrained.transpose(0, 1), mask_features, self.training or (i == len(hs)-1), self.pretrained_weight)
                predictions_class.append(outputs_class)
                predictions_mask.append(outputs_mask)
        else:
    ...
    def forward_prediction_heads_concat(self, output, output_pretrained, mask_features, pred_mask=True):
        decoder_output = self.decoder_norm(output)
        decoder_output = decoder_output.transpose(0, 1)
        decoder_output_pretrained = self.decoder_norm(output_pretrained)
        decoder_output_pretrained = decoder_output_pretrained.transpose(0, 1)
        outputs_class = self.class_embed(torch.cat([decoder_output, decoder_output_pretrained], dim=-1))
        outputs_mask = None
        if pred_mask:
            mask_embed = self.mask_embed(decoder_output)
            outputs_mask = torch.einsum("bqc,bchw->bqhw", mask_embed, mask_features)
    ...
    def forward_prediction_heads_weight(self, output, output_pretrained, mask_features, pred_mask=True, weight_pret=0.5):
        decoder_output = self.decoder_norm(output)
        decoder_output = decoder_output.transpose(0, 1)
        decoder_output_pretrained = self.decoder_norm(output_pretrained)
        decoder_output_pretrained = decoder_output_pretrained.transpose(0, 1)
        outputs_class = self.class_embed(decoder_output) 
        outputs_class_pretrained = self.class_embed(decoder_output_pretrained)
        outputs_class = outputs_class * (1-weight_pret) + outputs_class_pretrained * weight_pret
        outputs_mask = None
        if pred_mask:
            mask_embed = self.mask_embed(decoder_output)
            outputs_mask = torch.einsum("bqc,bchw->bqhw", mask_embed, mask_features)
