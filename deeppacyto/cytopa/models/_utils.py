...
def box_roi_pool_cell(target_cell, proposals, thres=0.5):
    # target_cell: length [bs]에 'boxes', 'labels', 'scores', 'masks'로 구성
    # proposals: [bs] 크기, 각 항목은 [proposals[0].shape[0], 4] 크기의 ROI 정보
    bs = len(target_cell)
    device = target_cell[0]['scores'].device
    result = [] #torch.zeros((bs, proposals[0].shape[0], 2), device=device)  # [bs, proposals[0].shape[0], 2] 크기 (마스크 개수, 평균 넓이)
    ...
    for i in range(bs):
        ...
        roi = proposals[i]  # [proposals[0].shape[0], 4] 크기 (ROI 정보)
        boxes = target_cell[i]['boxes']  # [22, 4] 크기 (각 마스크의 bounding box 정보)

        # score가 thres 이상인 마스크만 필터링
        valid_masks_indices = scores > thres # [22] 크기, thres 이상인 인덱스를 True로 설정
        valid_masks = masks[valid_masks_indices]  # [valid_count, 1, 800, 800]
        valid_scores = scores[valid_masks_indices]  # [valid_count]
        valid_boxes = boxes[valid_masks_indices]  # [valid_count, 4] 크기

        num_valid_masks = valid_masks.size(0)  # 유효한 마스크의 개수
        if num_valid_masks == 0:
            # 유효한 마스크가 없으면 (0, 0) 저장 (proposals[0].shape[0] proposal에 대해)
            # result[i] = torch.tensor([0, 0], device=device).repeat(roi.size(0), 1)
            result.append(torch.tensor([0, 0], device=device).repeat(roi.size(0), 1))   

            continue

        num_contained = torch.zeros(roi.size(0), device=device)  # 50% 이상 포함되는 마스크의 개수
        total_area = torch.zeros(roi.size(0), device=device)  # 마스크들의 평균 넓이를 계산하기 위한 변수

        # 각 proposal에 대해 마스크가 50% 이상 포함되는지 확인
        for j in range(num_valid_masks):
            mask = valid_masks[j].squeeze(0)  # [800, 800]
            mask_area = mask.sum().item()  # 마스크의 넓이 (1인 영역의 개수)
            if mask_area == 0:   # 라벨 1 없을 경우 skip
                continue
            box = valid_boxes[j]  # 각 마스크의 bounding box 정보 [x1, y1, x2, y2]
            # proposal 영역에 대해 마스크가 50% 이상 포함되는지 계산
            for k in range(roi.size(0)):  # 1000개의 proposal을 순회
                x1, y1, x2, y2 = roi[k]  # 각 proposal의 (x1, y1, x2, y2) 좌표

                # ROI와 마스크의 bounding box가 겹치는지 확인
                mask_x1, mask_y1, mask_x2, mask_y2 = box  # 마스크의 bounding box [x1, y1, x2, y2]
                if x2 < mask_x1 or x1 > mask_x2 or y2 < mask_y1 or y1 > mask_y2:
                    # 겹치지 않으면 건너뛰기
                    continue
                # 교집합 영역이 있다면 계산
                proposal_area = (x2 - x1) * (y2 - y1)  # proposal 영역의 넓이
                mask_roi = mask[int(y1):int(y2), int(x1):int(x2)]  # 해당 ROI에 해당하는 마스크 영역
                intersection_area = mask_roi.sum().item()  # 교집합 영역의 넓이

                # 교집합 영역이 proposal 영역의 50% 이상이면 포함된 마스크로 처리
                if intersection_area / mask_area >= 0.5:
                    num_contained[k] += 1
                    total_area[k] += mask_area  # 해당 마스크의 넓이를 추가
                '''
                num_contained[k] += 1
                total_area[k] += mask_area  # 해당 마스크의 넓이를 추가
                '''

        # 결과 저장 (1000개의 proposal에 대해 계산된 결과)
        #average_area = total_area / num_contained if num_contained > 0 else 0
        # 평균 넓이 계산
        average_area = torch.where(
            num_contained > 0,
            total_area / num_contained,
            torch.zeros_like(total_area)
        )
        
        if normalize != None:
            normalize_average_area = torch.where(
                average_area > 0,
                average_area / (normalize['area'] * masks.shape[-1] * masks.shape[-2]), #Normalize area : per pixel
                torch.zeros_like(average_area)
            )
            normalize_num_contained = torch.where(
                num_contained > 0,
                num_contained / normalize['count'],
                torch.zeros_like(num_contained)
            )
            average_area = normalize_average_area
            num_contained = normalize_num_contained
        tmp = torch.zeros((roi.size(0), 2), device=device)  # [bs, proposals[0].shape[0], 2] 크기 (마스크 개수, 평균 넓이)
        tmp[:,0] = num_contained
        tmp[:,1] = average_area
        result.append(tmp)
        # result[i, :, 0] = num_contained  # 각 proposal에 대해 포함된 마스크의 개수
        # result[i, :, 1] = average_area  # 각 proposal에 대해 마스크들의 평균 넓이
