...
class CUTModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        """
        parser.add_argument('--CUT_mode', type=str, default="cut_m2", choices='(CUT, cut, FastCUT, fastcut, cut_m1, cut_m2, cut_m3)')
        ...
        elif opt.CUT_mode.lower() == "cut_m1":
            parser.set_defaults(nce_idt=True, lambda_NCE=10.0)
        elif opt.CUT_mode.lower() == "cut_m2":
            parser.set_defaults(nce_idt=True, lambda_NCE=1.0, nce_layers='0,4,8')
        elif opt.CUT_mode.lower() == "cut_m3":
            parser.set_defaults(nce_idt=True, lambda_NCE=10.0, nce_layer='0,4,8')
        else:
    ...
    def __init__(self, opt):
        ...
        if self.opt.isX:
            if self.isTrain:
                ...
                self.visual_names = ['fake_B']#['real_A', 'fake_B', 'real_B','real_A_sig','fake_B_sig','real_B_sig', 'fake_B_cam_IntegratedGradients','real_B_cam_IntegratedGradients']
        else:
    ...
    def forward(self):
        ...
        if self.opt.isX:
            ...
            threshold_A = self.opt.salA    #PA90 45
            threshold_B = self.opt.salB   #HE 170 PAP
    ...
    def compute_G_loss(self):
        ...
        if self.opt.isX:
            ...
            self.loss_G = self.loss_G_GAN + loss_NCE_both + self.loss_content * self.opt.w_X
        else:
