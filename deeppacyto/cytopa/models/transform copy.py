...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def forward(
        self, images: List[Tensor], targets: Optional[List[Dict[str, Tensor]]] = None, target_cells: Optional[List[Dict[str, Tensor]]] = None
        ...
        if target_cells is not None:
            target_cells = [tc for tc in target_cells]
        for i in range(len(images)):
            ...
            if target_cells is not None:
                ...
                target_cell, target_index = self.resize(target_cell, target_index)
        ...
        if target_cells is not None:
            target_cells = self.batch_images(target_cells, size_divisible=self.size_divisible)
            target_cells_list = ImageList(target_cells, image_sizes_list)
            return image_list, targets, target_cells_list
        else:
