...
def _create_text_labels(classes, scores, class_names, is_crowd=None, stats=None):
    """
    Args:
    ...
    if stats is not None:
        labels = ["{} \ne{} s{:.2f}".format(l, s[0], s[1]) for l, s in zip(labels, stats)]
...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions, jittering: bool = False):
        ...
        Args:
            ...
            jittering (bool): whether to randomly jitter the colors. If False, uses exact colors.
        ...
        if hasattr(self.metadata, 'thing_colors') and self.metadata.thing_colors:
            if jittering:
                colors = [
    ...
    def overlay_instances(
        ...
        *,
        ...
        if assigned_colors is None:
            default_color = [1.0, 0.5, 0]
            # assigned_colors = [random_color(rgb=True, maximum=1) for _ in range(num_instances)
            assigned_colors = [self._jitter(default_color) for _ in range(num_instances)]
        if num_instances == 0:
            return self.output
